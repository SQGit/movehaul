...
public class FetchAddressIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (location == null) {
            errorMessage = getString(com.movhaul.customer.R.string.no_location_data_provided);
        ...
        try {
            ...
            errorMessage = getString(com.movhaul.customer.R.string.service_not_available);
            Log.e(TAG, errorMessage, ioException);
        } catch (IllegalArgumentException illegalArgumentException) {
            // Catch invalid latitude or longitude values.
            errorMessage = getString(com.movhaul.customer.R.string.invalid_lat_long_used);
        ...
        if (addresses == null || addresses.size() == 0) {
            if (errorMessage.isEmpty()) {
                errorMessage = getString(com.movhaul.customer.R.string.no_address_found);
                Log.e(TAG, errorMessage);
            }
            deliverResultToReceiver(AppUtils.LocationConstants.FAILURE_RESULT, errorMessage, null);
        } else {
